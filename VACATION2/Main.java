...
public static void main(String[] args)
{
	...
	for(int i=0;i<N;i++)
	{
		...
		{
			if(i<K || j<K)
				{dpk[i][j] =dp[N][i][j];continue;}
			dpk[i][j]=-1;
			for(int k=0;k<K;k++)
			{
				if(dp[N][i][k]!=-1 && dp[N][k][j]!=-1) 
				{
					if(dpk[i][j]==-1) dpk[i][j] = dp[N][i][k] + dp[N][k][j];
					else dpk[i][j] =min(dpk[i][j], dp[N][i][k] + dp[N][k][j]);
				}
			}
